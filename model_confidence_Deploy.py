...
# 4. 计算 Model Confidence 置信度分数
model_confidence = ModelConfidence(model, X_test, y_test)
model_conf_scores = model_confidence.compute_confidence(X_test, y_test)

# 5. 按置信度排序
sorted_indices = np.argsort(model_conf_scores)
sorted_confidence_scores = model_conf_scores[sorted_indices]
...
for percentile in percentiles:
    ...
    if len(percentile_y_test) > 0:
        ...
        model_conf_precisions.append(precision)
    else:
        model_conf_precisions.append(0)  # 如果样本为空，则精度设为 0
...
plt.title("Precision vs. Percentile for Model Confidence")
plt.legend()
plt.grid()
plt.show()
